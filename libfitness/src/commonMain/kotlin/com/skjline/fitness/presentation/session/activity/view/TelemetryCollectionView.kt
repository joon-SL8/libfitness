...
fun TelemetryCollectionView(
    ...
    LazyVerticalGrid(
        modifier = modifier,
        columns = GridCells.Fixed(count = countColumns),
        ...
        content = {
            ...
            items(availableServices.size) { index ->
                ...
                collectionService.getProviderOf(service)?.let { provider ->
                    val predicate: (DataPacket) -> Boolean = { p ->
                        p::class.simpleName == service.name
                    }

                    TelemetryComponentView(
                        modifier = itemModifier,
                        label = availableServices[index].getStringResource(),
...
@Composable
private fun PacketType.getStringResource(): String = when (this) {
